...
% request_pool_size_list = 4;
% request_pool_size = 8;
for request_pool_size_index = 1:length(request_pool_size_list)
request_pool_size = request_pool_size_list(request_pool_size_index);
...
% % % locality_list = 100:-1:80;
% % % locality_list = [20:59 101:160];
% locality_list = [0:59];
% locality_list = [300:-1:101];
% locality_list = 280:340;
locality_list = 0:340;
% locality_list = 90;
...
% jobid_list = 1000;
% jobid_list = 1e3+1;
% % % % jobid_list = 1e5+1; for chapter3, vary l 
jobid_list = 1662;
% jobid_list = 2:5;
% jobid_list = 1:5;
% jobid_list = 6:10;
...
filename = 'final_localities_PredictionTime_del5.mat';
% filename = 'final_localities_TrainError_del1.mat';
load(filename);
locality_list = final_locality_g(request_pool_size_index);
for index_iter = 1:1
% for index_iter = 2:10
for rho = rho_list
    ...
    spmd(request_pool_size)
        ...
        switch data_kind
            ...
            case 'KS'
                ...
                % L = 100; N = 256; 
                test_steps = 20000; 
                % train_steps = 5000; test_steps = 20000; 
                % train_steps = 10000; test_steps = 20000; 
        ...
        switch learn
            ...
            case 'LSM_GD_short_prediction_time'
                ...
                while iter <= max_iter
                    ...
                    % for dlocality = 2:-1:1
                        ...
                        switch l
                            case 1
                                ...
                                beg = 1;
                                for i=1:num_inputs2
                                    rng((jobid*max_iter*num_workers+iter*num_workers+l)*num_inputs2+i);
                                    % rng(i);
                                    % ip = (-1 + 2*rand(q,1));
                                    % win((i-1)*q+1:i*q,i) = ip;
                                    ip = (-1 + 2*rand(nodes_list(i), 1));
                        ...
                        % pred_collect = res_predict(x, wout, A, win, test_data(1:2*locality+chunk_size_data, :), resparams, jobid, rear_locality_data, forward_locality_data, n_kind_data, chunk_size_data, num_reservoirs_per_worker, pred_marker_array, sync_length, transpose(test_in));
                        % [pred_collect, deltas] = res_predict_GD(L, x, wout, A, win, transpose(test_u), resparams, jobid, rear_locality_data, forward_locality_data, n_kind_data, chunk_size_data, num_reservoirs_per_worker, pred_marker_array, sync_length, test_in);
                        [pred_collect, deltas] = res_predict_GD(L, x, wout, A, win, test_data(1:2*locality+chunk_size_data, :), resparams, jobid, rear_locality_data, forward_locality_data, n_kind_data, chunk_size_data, num_reservoirs_per_worker, pred_marker_array, sync_length, test_in);
                    ...
                    bef = size(trajectories_true);
                    trajectories_true = gcat(trajectories_true, 1, 1);
                    aft = size(trajectories_true);
                    diff = gcat(diff, 1, 1);
                    % filename = [data_dir '/', data_kind, '/', data_kind, '_result_' learn '_uniform_train', num2str(train_steps), '_node', num2str(approx_reservoir_size) '-L' num2str(L) '-radius' num2str(rho) '-locality' num2str(locality) '-numlabs' num2str(num_workers) '-jobid' num2str(jobid) '-index_iter', num2str(which_index_iter) '.mat'];
                    % filename = [data_dir '/', data_kind, '/', data_kind, 'result_linear_train', num2str(train_steps), '_node', num2str(approx_reservoir_size) '-L' num2str(L) '-radius' num2str(rho) '-locality' num2str(locality) '-numlabs' num2str(num_workers) '-jobid' num2str(jobid) '-index_iter', num2str(which_index_iter) '.mat'];
...
%    filename = ['KS100-' num2str(approx_reservoir_size) '-locality' num2str(locality) '-numlabs' num2str(num_workers) '-jobid' num2str(jobid) '-index_iter', num2str(which_index_iter)];
    ...
    % if total > 1
    %     % clearvars -except request_pool_size request_pool_size_list h rho_list locality_list width_list train_steps_list jobid_list;
    %     clearvars states win wout A trajectories_true pred_collect diff deltas test_u u data locations times test_file test_data
    % end
    if total > 1
        clearvars -except cost_function data_dir data_kind full_pred_marker_array h index_file indices_per_job jobid_list learn locality_list num_divided_jobs num_indeces num_reservoirs num_reservoirs_per_worker request_pool_size request_pool_size_list reservoir_kind rho_list save_results show_fig train_steps_list width_list index_iter rho locality_ width train_steps jobid;
    end
end
end
end
