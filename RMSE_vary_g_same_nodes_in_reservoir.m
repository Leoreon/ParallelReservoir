...
data_kind = 'KS'; L = 22; dt = 1/4; num_workers_list = [2 3 4 5 6 7 8]; Ntotal_list = 5040 * num_workers_list; N=840; train_steps = 80000; locality_list = 80; jobid_list = 1:5; % locality_list = [20 25 30 35 40 45 50 55 60]; num_workers_list = [1 4 5 6 7 8 10]; % num_workers_list = [1 2 4 5 6 8 10 12];
...
for h = 1:length(locality_list)
    locality = locality_list(h);
    % train_errors = zeros(length(jobid_list), length(locality_list));
    train_errors = zeros(length(jobid_list), length(num_workers_list));
    if show_error_plot
        figure();
    end
    errors = zeros(length(num_workers_list), pred_length);
    for k = 1:length(num_workers_list)
        num_workers = num_workers_list(k);
        Ntotal = Ntotal_list(k);
        ...
        if show_error_plot
            ...
            plot(times(1,1:pred_length), errors(k, 1:pred_length), 'LineWidth', lineWidth, 'DisplayName', ['g=' num2str(num_workers)]); 
