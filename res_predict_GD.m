...
function pred_collect = res_predict_GD(x, w_out, w, w_in, test_data, resparams, jobid, rear_locality_data, forward_locality_data, n_kind_data, chunk_size, num_reservoirs_per_worker, pred_marker_array, sync_length, test_in)
...
for pred_iter = 1:num_preds
    ...
    prediction = predict_GD(w,w_out,x,w_in,resparams.predict_length,chunk_size, num_reservoirs_per_worker, frontWkrIdx, rearWkrIdx,resparams.N, rear_locality_data, forward_locality_data, n_kind_data, test_in(:, prediction_marker+sync_length+1:inputExist*(prediction_marker+sync_length+resparams.predict_length)));
