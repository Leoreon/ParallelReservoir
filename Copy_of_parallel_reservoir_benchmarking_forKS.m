...
% train_steps_list = [2500 5000 10000]; test_steps = 20000;
% rho_list = 0.2:1:1.7;
% locality_list = 3:4:8;
jobid_list = 1;
% jobid_list = 1:2;
    ...
    spmd(request_pool_size)
        ...
        switch data_kind
            ...
            case 'KS'
                ...
                % L = 50; N = 2048; 
                % L = 100; N = 256; 
                % train_steps = 1250; test_steps = 20000; 
                % train_steps = 2500; test_steps = 20000; 
...
%        tf = matfile('KS100/test_input_sequence.mat');
        ...
        % resparams.predict_length = 10000-sync_length-1;  %number of steps to be predicted
        ...
        switch learn
            case 'LSM_GD_short_prediction_time'
                ...
                while iter <= max_iter
                    ...
                    % for dlocality = 2:-1:1
                        locality = locality + (dlocality_index-1);
                        num_inputs2 = chunk_size_data + 2 * (n_kind_data + n_kind_input) * locality;
                        switch l
                            case 1
                                fprintf('locality: %d\n', locality);
                                rng(jobid+iter*2+dlocality_index);
                                % [num_inputs2,~] = size(u.');
...
%% //////////////////////////////////////////////////
            case 'LSM_GD_training_error'
                ...
                while iter <= max_iter
                    ...
                    l_list(iter) = locality;
                    currentAndNextT = zeros(2, 1);
                    currentAndNextError = zeros(2, 1);
                    d_locality = 1;
                    dlocality_list = [0 d_locality];
                    for dlocality_index = 1:2
                    % for dlocality = 2:-1:1
                        dlocality = dlocality_list(dlocality_index);
                        locality = locality + dlocality_index;
                        num_inputs2 = chunk_size_data + 2 * (n_kind_data + n_kind_input) * locality;
                        % switch l
                        %     case 1
                        fprintf('locality: %d\n', locality);
            ...
            case 'LSM_common'
                ...
                if feedback_pred
                    ...
                    states = [];
                    test_data = transpose(test_u(:, 1:chunk_size_data+(rear_locality_data+forward_locality_data)));
                    states_pred_data = reservoir_layer_pred(x, A, win, test_data, resparams, train_steps, transpose(train_in(:, 1:chunk_size_input+(rear_locality_input+forward_locality_input))), sync_length);
                    
                    % error = wout*states - data(n_kind_data*locality+1:n_kind_data*locality+chunk_size_data,resparams.discard_length + 1:resparams.discard_length + resparams.train_length);
                    pred_collect = wout*states_pred_data;
                    error_pred_data = pred_collect - test_data(rear_locality_data+1:rear_locality_data+chunk_size_data,sync_length + 1:end);
                    error_pred_data = error_pred_data .^ 2;
                    RMSE_pred_data = sqrt(mean(mean(error_pred_data)));
                    error_pred_data = gcat(error_pred_data);
                    trajectories_true = gcat(test_data(rear_locality_data+1:rear_locality_data+chunk_size_data, sync_length+1:end));
                    RMSE_pred_data = gcat(RMSE_pred_data, 1);
                end
                collated_prediction = gcat(pred_collect,1,1);
            case 'LSM'
                if iter
    ...
    switch learn
        ...
        otherwise
            if feedback_pred
                ...
                trajectories_true = trajectories_true{1};
                diff = error_pred_data{1};
                error = sqrt(mean(diff.^2, 1));
                RMSE_pred_data = RMSE_pred_data{1};
            end
    end
...
%    filename = ['KS100-' num2str(approx_reservoir_size) '-locality' num2str(locality) '-numlabs' num2str(num_workers) '-jobid' num2str(jobid) '-index_iter', num2str(which_index_iter)];
    ...
    if jobid == 0
        ...
        if feedback_pred
            ...
            % RMSEs and RMSE_pred_data
            save(filename, 'error', 'resparams', 'RMSE_mean', 'pred_marker_array', 'locality', 'chunk_size_data', 'runtime', 'RMSE_pred_data', '-v7.3');
        end
    end
    display(filename);
