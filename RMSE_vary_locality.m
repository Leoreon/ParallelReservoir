...
% L = 44; Ntotal = 5040; locality_list = 10:10:80; num_workers_list = [2 3 4 5 6 7 8]; % locality_list = [20 25 30 35 40 45 50 55 60]; num_workers_list = [1 4 5 6 7 8 10]; % num_workers_list = [1 2 4 5 6 8 10 12];
...
for h = 1:length(num_workers_list)
    hold on;
    num_workers = num_workers_list(h);
    plot(locality_list, T_list(h, :), 'DisplayName', ['g=' num2str(num_workers)]);
    hold off;
end
sgtitle(['L=' num2str(L) ', num reservoirs: ' num2str(num_workers) ', Ntotal=' num2str(Ntotal)]);
xlabel('locality'); ylabel('short-term prediction time');
