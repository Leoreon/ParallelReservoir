...
% request_pool_size_list = 7:8;
request_pool_size_list = 2:2:8;
...
% locality_list = [50 55];
% locality_list = 60:160;
% locality_list = 60:140;
% locality_list = 60:70;
...
% % % jobid_list = 1e4 + [1:5];
jobid_list = 1e2 + 1;
% jobid_list = 1e5 + [1:10]; %% change predict_length before changing jobids!!
% jobid_list = 1e5 + [11:20]; %% change predict_length before changing jobids!!
% jobid_list = 1e5 + [21:30]; %% change predict_length before changing jobids!!
% jobid_list = 1e5 + [41:80]; %% change predict_length before changing jobids!!
% jobid_list = 2:5;
% jobid_list = 1:5;
% jobid_list = 1:10;
% jobid_list = 6:10;
...
for jobid = jobid_list
    ...
    % partial_pred_marker_array = full_pred_marker_array(1:10);
...
%        tf = matfile('KS100/test_input_sequence.mat');
        ...
        % resparams.predict_length = 2000/4/dt-sync_length-1;  %number of steps to be predicted
        resparams.predict_length = 3000/4/dt-sync_length-1;  %number of steps to be predicted
        
        % resparams.predict_length = 5000/4/dt-sync_length-1;  %number of steps to be predicted
        
        % resparams.predict_length = 10000/4/dt-sync_length-1;  %number of steps to be predicted
        % resparams.predict_length = 10000/4/dt-sync_length-1;  %number of steps to be predicted
        % resparams.predict_length = 20000/4/dt-sync_length-1;  %number of steps to be predicted
        ...
        switch learn
            case {'LSM_GD_short_prediction_time', 'LSM_GD'}
                if true
                    ...
                    test_u = train_uu(50001:70000,:);
                    fprintf('use train data for validation\n');
                end
            otherwise
                        ...
                        case 'Adam'
                            ...
                            switch l
                                case 1
                                    ...
                                    % fprintf('  gradE: %f\n', gradT);
                                    fprintf('  Cost(%d)=%f\n  Cost(%d)=%f\n', current_locality+del_locality, nextCost, current_locality, currentCost);
                                    fprintf('  gradE: %f\n', gradE);
            ...
            case 'LSM_GD_short_prediction_time'
                ...
                while iter <= max_iter
                    ...
                    % for dlocality = 2:-1:1
                        ...
                        switch pred_criteria
                            case 'average'
                                mean_error = zeros(size(RMSE_pred(1:resparams.predict_length)));
                                
                                for k = 1:num_preds
                                    mean_error = mean_error + RMSE_pred(resparams.predict_length*(k-1)+1:resparams.predict_length*k);
                                    out_times = find(RMSE_pred(resparams.predict_length*(k-1)+1:resparams.predict_length*k) > threshold);
                                    out_time = out_times(1);
                                    currentAndNextT(dlocality_index) = currentAndNextT(dlocality_index) + out_time;
                                end
                                % display('kk');
    ...
    switch learn
        ...
        case 'LSM_GD_short_prediction_time'
            ...
            if show_fig
                ...
                xlabel('k'); ylabel('\Delta_k'); grid on;
                % figure(); plot(Lambdas); 
...
%    filename = ['KS100-' num2str(approx_reservoir_size) '-locality' num2str(locality) '-numlabs' num2str(num_workers) '-jobid' num2str(jobid) '-index_iter', num2str(which_index_iter)];
    ...
    switch reservoir_kind 
        case {'uniform' 'uniform_fix'}
            if resparams.beta == 0.0001
                ...
                filename = [data_dir '/', data_kind, '/', data_kind, '_result_' learn '_' reservoir_kind '_reservoir' '_beta' num2str(resparams.beta) '_train', num2str(train_steps), '_node', num2str(approx_reservoir_size) '-L' num2str(L) '-N' num2str(N) '-radius' num2str(rho) '-locality' num2str(locality) '-numlabs' num2str(num_workers) '-jobid' num2str(jobid) '-index_iter', num2str(which_index_iter) '.mat'];
            end
    ...
    %% show graphs
    n_steps = size(trajectories_true, 2) / num_preds;
    ...
    total = size(request_pool_size_list, 2) * size(width_list, 2) * size(rho_list, 2) * size(locality_list, 2) * size(jobid_list, 2) * size(train_steps_list, 2);
    
    if show_fig || total < 2 || true
        max_time = 30; max_steps = round(max_time / dt / max_lyapunov);
        % max_time = 60; max_steps = round(max_time / dt / max_lyapunov);
        switch data_kind 
            case {'KS', 'KS_small_step'}
                ...
                subplot(3, 1, 3); surf(times, locations, diff(:,1:n_steps)); view(0, 90); shading interp, axis tight; xlabel('\Lambda_1t'); ylabel('x'); colorbar; clim(2 * [min_value max_value]); xlim([0 max_time]); %title('error');
                ...
                filename_3_big = ['chapter2/true_predict_error_L' num2str(L) '_g' num2str(num_workers) '_l' num2str(locality) '_Dr' num2str(approx_reservoir_size) '_jobid' num2str(jobid) '_big.jpg'];
                print(gcf, '-djpeg', filename_3_big, '-r600');
                % 
                for k = 1:3
                    subplot(3, 1, k);
                    xlim([max_time-10 max_time]);
                    % xlim([0 20]);
                end
                filename_3_small = ['chapter2/true_predict_error_L' num2str(L) '_g' num2str(num_workers) '_l' num2str(locality) '_Dr' num2str(approx_reservoir_size) '_jobid' num2str(jobid) '_small.jpg'];
        ...
        figure(); plot(times(1,:), sqrt(mean(diff(:, 1:n_steps).^2, 1)));
        xlabel('time (lyapunov*second'); ylabel('RMSE'); title('error');
        
        figure(); plot(times(1, 1:max_steps), deltas(1, 1:max_steps)); 
        xlabel('\Lambda_1t'); ylabel('\Delta(\Lambda_1t)'); fontsize(11, 'points');
        filename_delta = ['chapter2/delta_L' num2str(L) '_g' num2str(num_workers) '_l' num2str(locality) '_Dr' num2str(approx_reservoir_size) '_jobid' num2str(jobid) '.jpg'];
        print(gcf, '-djpeg', filename_delta, '-r600');
    end
...
% figure(); plot(times(1, 1:size(deltas, 2)), deltas(1, :)); xlim([0 20]); xlabel('lyapunov time'); ylabel('\Delta(t)'); fontsize(16, 'points');
